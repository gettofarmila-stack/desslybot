...
def voucher_history_builder(vouchers, page: int=0):
    builder = InlineKeyboardBuilder()
    page_count = 5
    start_idx = page * page_count
    end_idx = start_idx + page_count
    current_vouchers = vouchers[start_idx:end_idx]
    for voucher in current_vouchers:
        builder.row(types.InlineKeyboardButton(text=voucher.voucher_name, callback_data=f'voucher_history_select_{voucher.id}', style='primary'))
    nav_buttons = []
    if page > 0:
        nav_buttons.append(types.InlineKeyboardButton(text='⬅️ Назад', callback_data=f'voucher_page_{page - 1}', style='danger'))
    if end_idx < len(vouchers):
        nav_buttons.append(types.InlineKeyboardButton(text='Вперед ➡️', callback_data=f'voucher_page_{page + 1}', style='danger'))
    if nav_buttons:
        builder.row(*nav_buttons)
    builder.row(types.InlineKeyboardButton(text='К категориям', callback_data='order_history', style='danger'))
    builder.row(types.InlineKeyboardButton(text='Главное меню', callback_data='main_menu', style='danger'))
    return builder.as_markup()

def voucher_history_select_builder():
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text='Назад', callback_data='voucher_order_history', style='primary'))
    builder.row(types.InlineKeyboardButton(text='Главное меню', callback_data='main_menu', style='danger'))
    return builder.as_markup()