...
async def create_steam_topup_order(customer_id, login: str, amount: float):
    ...
    info = await create_steam_topup_order_api(customer_id=customer_id, login=login, amount=amount)
    return f'Заказ ID:{info.get('uuid')} успешно создан!'

async def check_steam_login(login, amount):
    if amount < 0.1:
        raise BotError('Слишком маленькая сумма пополнения!')
    can_refill = await check_steam_login_api(login, amount)
    return can_refill
