...
async def check_payment_api(uuid, order_id):
    url = 'https://api.2328.io/api/v1/payment/info'
    data = {'uuid': uuid, 'order_id': str(order_id)}
    json_str = json.dumps(data, separators=(',', ':'), ensure_ascii=False)
    sign = generate_sign(data)
    headers = {'Content-Type': 'application/json', 'project': PROJECT_UUID, 'sign': sign}
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=json_str.encode('utf-8'), headers=headers) as response:
                data = await response.json()
                if data.get('state') == 0:
                    return data['result']
                raise BotError('Ошибка проверки платежа!')
    except Exception as e:
        logging.error(f'Ошибка check_payment_api: {data}')
        raise BotError('Ошибка АПИ платёжки!')